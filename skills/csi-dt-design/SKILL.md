...
在设计 Oracle、fixture 和 test double 时，执行以下 Gate 检查，任一问题无法正面回答时先修订 DT 设计，不把问题留给下一阶段：

- **真实行为 Gate**：当前断言验证的是公开行为、稳定状态或正式契约，而不是 mock 是否存在、被调用次数是否恰好符合当前实现细节；
- **依赖理解 Gate**：已经说明真实依赖的关键副作用，确认当前测试不会因为错误 mock 掉这些副作用而失去判别力；
- **测试专用接口 Gate**：没有为了测试方便而默认要求生产代码新增 test-only 方法、调试开关或绕过正式行为的入口；
- **最小命题 Gate**：每个 automated DT 都能明确回答“这个 Case 主要证明什么，不证明什么”，避免一个 Case 混合过多目标；
- **Red 可观察性 Gate**：对尚未实现的行为，已经说明预实现阶段预期会看到的失败信号，以及什么情况属于“失败原因不对”。

以下信号默认视为弱 DT 设计，需要在本阶段拆分或重写：

- Oracle 只能断言 mock、stub 或日志文本本身；
- 依赖替代范围大于验证目标，导致核心链路被整体替换；
- Case 名称无法直接说明行为或包含多个并列目标；
- 需要引入生产测试专用 API 才能观测目标行为；
- 预实现阶段即使测试通过，也无法解释它为什么仍然具备判别力。
