...
1. 从 DT 基线中选择当前 task 直接对应的 `valid_red` 或需要保持的 `valid_green`，按 Case 或最小 Case 组推进，不一次性铺开所有实现；
2. 在当前 Case 组上实现满足设计契约和任务完成条件的最小变更；
3. 先复跑当前 Case 组，确认 Red 变 Green 或既有 Green 保持稳定，再运行与本次编辑直接相关的编译、静态检查或快速检查；
4. 更新任务映射、实际变更、围栏判定和验证证据；
5. 逐个完成剩余任务，保持接口、迁移、兼容和回滚顺序；
6. 对照变更基线复核 Changed Files，区分本次增量与 `pre_existing` 用户改动。

实施阶段同时遵守以下测试纪律：

- 没有对应失败测试或可信 Green 证据的生产代码，不进入实现；
- 当前 Case 失败原因不清时，先回到 DT 修正测试资产，不把问题带着继续写代码；
- 写代码只为让当前 Case 组通过，不顺手扩展未经 DT 覆盖的新行为；
- 当前 Case 组未转绿前，不宣称 task 完成；
- 如果发现自己实际上是在“先写实现，稍后统一跑测试”，应停止当前实现并回到按 Case 推进的节奏。

实现优先复用现有模块、错误处理、配置和数据模式。新增文件或抽象记录其设计锚点和必要性。生产代码、运行配置、迁移与正式文档由 task 和围栏共同授权；测试资产修订通过 DT 回流重新建立基线。

所有 task 已完成、每项本次变更均为围栏合规或当前运行有效的一次性例外、当前 task 对应的 Case 组已有可追溯的转绿或稳定 Green 证据、快速检查有证据且没有上游缺口时，实施产物才设置为 `ready`。
