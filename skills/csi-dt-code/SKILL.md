...
生成每个 automated Case 的测试代码时，同时执行以下质量约束：

- 一个测试名称只描述一个核心行为；名称出现多个并列行为时，优先拆分；
- 优先使用真实代码路径，mock 仅用于不可控外部边界或成本明显不合理的依赖；
- 在新增 mock 或 fixture 前，先说明真实依赖有哪些副作用、当前 Case 依赖哪些副作用、为什么此处 mock 不会削弱 Oracle；
- 不把测试清理、状态注入或观测便利性直接做成生产代码中的 test-only 方法；
- 断言优先体现目标 API、状态不变量或外部契约，避免围绕实现细节搭建脆弱断言。
...
对每个 automated Case，按以下顺序做 Red/Green 有效性校验，而不是只记录最终退出状态：

1. **Verify RED**：先观察测试在当前实现下的真实结果；
2. 若失败，确认失败原因是目标行为缺失、契约不满足或正式错误路径未实现，而不是语法、导入、fixture、命令或拼写问题；
3. 若报错或失败原因错误，先修测试资产并重跑，直到得到可解释的失败或可信 Green；
4. 若直接通过，只能在补充了反例、边界、协议不变量或等价敏感性证据后判定为 `valid_green`；无法证明判别力时按 `invalid_test` 处理；
5. 记录本 Case 的失败/通过证据，避免只给出整体命令级结论。

以下信号默认禁止直接进入 `ready`：

- 新增测试从未被单独观察过；
- 无法解释某个失败为什么属于目标行为缺口；
- 测试直接通过，但没有证据证明 Oracle 对错误实现也会敏感；
- 测试依赖 test-only 生产接口、临时硬编码或未说明的环境前提；
- 关键断言实际只验证 mock 行为或辅助对象存在。
...
当 Case 结果记录为 `valid_green` 时，正文必须额外说明：为什么该 Case 不是“测试已有行为却无法发现错误”的弱测试，以及若实现偏离设计，哪个断言会先失效。
